...
function startDialDrag(e) {
    ...
    // The widget now lives inside the settings panel, which is hidden at page
    // load — so the center measured on DOMContentLoaded is stale/zero. Recompute
    // it now that the panel is actually on screen, or dragging maps to the wrong
    // angle.
    const rect = dialHandle.parentNode.getBoundingClientRect();
    dialCenter.x = rect.left + rect.width / 2;
    dialCenter.y = rect.top + rect.height / 2;
